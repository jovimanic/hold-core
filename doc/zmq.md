...
The HOLD Core daemon can be configured to act as a trusted "border
router", implementing the hold wire protocol and relay, making
...
The ZeroMQ feature in HOLD Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building holdd:
...
For instance:
    ...
    $ holdd -zmqpubhashtx=tcp://127.0.0.1:26699 \
               -zmqpubhashtx=tcp://192.168.1.2:26699 \
               -zmqpubrawtx=ipc:///tmp/holdd.tx.raw \
...
These options can also be provided in hold.conf.
...
From the perspective of holdd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into holdd directly. Furthermore, no information is
...
using. HOLDd appends an up-counting sequence number to each
