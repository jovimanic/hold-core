...
#include <qt/holdaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a HOLD address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new HOLDAddressEntryValidator(parent));
    widget->setCheckValidator(new HOLDAddressCheckValidator(parent));
}

bool parseHOLDURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no hold: URI
    if(!uri.isValid() || uri.scheme() != QString("hold"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!HOLDUnits::parse(HOLDUnits::HOLD, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseHOLDURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseHOLDURI(uriInstance, out);
}

QString formatHOLDURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("hold:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(HOLDUnits::format(HOLDUnits::HOLD, info.amount, false, HOLDUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openHOLDConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", HOLD_CONF_FILENAME));
    ...
    /* Open hold.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "HOLD.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "HOLD (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("HOLD (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "hold.desktop";
    return GetAutostartDir() / strprintf("hold-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a hold.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=HOLD\n";
        else
            optionFile << strprintf("Name=HOLD (%s)\n", chain);
