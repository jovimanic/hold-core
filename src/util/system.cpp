...
const char * const HOLD_CONF_FILENAME = "hold.conf";
const char * const HOLD_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/hold/hold/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //hold-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", HOLD_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\HOLD
    // macOS: ~/Library/Application Support/HOLD
    // Unix-like: ~/.hold
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "HOLD";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/HOLD";
#else
    // Unix-like
    return pathRet / ".hold";
#endif
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", HOLD_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure HOLD Core copyright is not removed by accident
    if (copyright_devs.find("HOLD Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The HOLD Core developers";
    }
