...
// hold includes.
#include <..\src\script\holdconsensus.h>
...
int main()
{
    std::cout << "holdconsensus version: " << holdconsensus_version() << std::endl;
    ...
    auto op0Result = holdconsensus_verify_script_with_amount(pubKeyScript.data(), pubKeyScript.size(), amount, (const unsigned char*)&stream[0], stream.size(), 0, holdconsensus_SCRIPT_FLAGS_VERIFY_ALL, &err);
